...
async function loadAQI() {
  try {
    ...
    if (
      ...
      console.warn("Invalid AQI data, using fallback");

      const fallbackAQI = localStorage.getItem("cityAQI") || 120;

      updateAQIUI(fallbackAQI);
      return;
    }
    ...
    const fallbackAQI = localStorage.getItem("cityAQI") || 100;
    updateAQIUI(fallbackAQI);

    document.getElementById("heroStatus").innerText = "Offline / Fallback Mode ⚠️";
  }
}
...
async function loadWeather() {
  try {
    ...
    document.getElementById("heroWind").innerText =
      result.data.windspeed ?? "--";

    // ✅ FIXED: show humidity from API
    document.getElementById("heroHumidity").innerText =
...
if(!result.data || !result.data.hourly)
  throw new Error("No chart data");

const hourlyAQI = result.data.hourly.us_aqi;
const hourlyTime = result.data.hourly.time;
...
async function loadPollutants(){
  try {
    ...
    if(!result.data || !result.data.hourly)
      throw new Error("No pollutant data");
